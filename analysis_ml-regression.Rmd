...
list.of.packages <- c('janitor', 'caret', 'elasticnet', 'ranger', 'randomForest')
...
dataframe_session1 <- clean_names(readxl::read_excel(path_datafile, sheet = 1, col_names = TRUE))
dataframe_questionnaires <- clean_names(readxl::read_excel(path_datafile, sheet = 2, col_names = TRUE, skip = 1))
dataframe_tlmetrics <- clean_names(readxl::read_excel(path_datafile, sheet = 1, col_names = TRUE))
```
...
table(c(colnames(dataframe_session1),
...
abmt_data = subset(abmt_data, select = -c(session, dob, subject_id))
