...
myfun_multiplot_specc <- function(df, min_clusters, max_clusters, id){
     ...
     if ( min_clusters < 2) stop('min_clusters must be > 1')
    # Initialize total within sum of squares: wss
    set.seed(50)
    wss <- 0
...
scaled_data_id <- cbind(data_frame[,1], scaled_data)
...
par(mfrow = c(2,2)) 
myfun_multiplot_km(data_pca2components, min_clusters = 1, max_clusters = 4, 'PCA2')
```
...
myfun_myscreeplot(wss_km_PCA2, min_clusters = 1, max_clusters = 4, 'K-Means Within Cluster Sum of Squares for \n1 Through 4 Clusters: \n with PCA2 Data')
...
par(mfrow = c(1,2)) 
myfun_boxplotbyclus(data_frame[,1], km_k2_res$cluster, data, 'K-Means PCA2 Clustering Solution: 2 Clusters, Raw Variable Scores')
myfun_boxplotbyclus(data_frame[,1], km_k3_res$cluster, data, 'K-Means PCA2 Clustering Solution: 2 Clusters, Raw Variable Scores')
```

#### Summary Statistics
```{r k-means cluster means and standard deviations}
(variable_means_km2 <- myfun_getmeans(km_k2_res$cluster, data, "K-means 2 Cluster Solution Developed on Data Reduced to 2 Principal Components: Raw Score Means by Cluster"))

(variable_means_km3 <- myfun_getmeans(km_k3_res$cluster, data, "K-means 3 Cluster Solution Developed on Data Reduced to 2 Principal Components: Raw Score Means by Cluster"))
```
...
kmeans_kcca <- flexclust::as.kcca(km_k2_res, data_pca2components)
```
...
par(mfrow = c(1,2)) 
myfun_multiplot_specc(data_pca2components, min_clusters = 2, max_clusters = 4, 'PCA2')
```
...
par(mfrow = c(1,2)) 
myfun_multiplot_specc(data_pca4components, min_clusters = 2, max_clusters = 4, 'PCA4')
```
...
par(mfrow = c(2,2)) 
myfun_myscreeplot(wss_specc_Scaled[2:4], min_clusters = 2, max_clusters = 4, 'Spectral Clustering Within Cluster Sum of Squares for \n2 Through 4 Clusters: \n with Scaled Data')
myfun_myscreeplot(wss_specc_PCA2[2:4], min_clusters = 2, max_clusters = 4, 'Spectral Clustering Within Cluster Sum of Squares for \n2 Through 4 Clusters: \n with PCA2 Data')
myfun_myscreeplot(wss_specc_PCA3[2:4], min_clusters = 2, max_clusters = 4, 'Spectral Clustering Within Cluster Sum of Squares for \n2 Through 4 Clusters: \n with PCA3 Data')
myfun_myscreeplot(wss_specc_PCA4[2:4], min_clusters = 2, max_clusters = 4, 'Spectral Clustering Within Cluster Sum of Squares for \n2 Through 4 Clusters: \n with PCA4 Data')
```
...
myfun_boxplotbyclus(data_frame[,1], spec_2clust_pca2, data, 'Spectral Clustering Solution: \n2 Clusters from 2 Component PCA Data, Raw Variable Scores')
```

#### Summary statistics
```{r spectral clustering solution means}
myfun_getmeans(spec_2clust_pca2, data, "Spectral Clustering 2 Cluster Solution Developed on Scaled Data: Raw Score Means by Cluster")
...
### Summary Statistics
```{r}
myfun_getmeans(hc_ward.D2_k2$cluster, data, "Hierarchical Complete Linkage 2 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")
myfun_getmeans(hc_complete_k2$cluster, data, "Hierarchical Complete Linkage 2 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")
myfun_getmeans(hc_average_k2$cluster, data, "Hierarchical Average Linkage 2 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")

myfun_getmeans(hc_ward.D2_k3$cluster, data, "Hierarchical Complete Linkage 3 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")
myfun_getmeans(hc_complete_k3$cluster, data, "Hierarchical Complete Linkage 3 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")
myfun_getmeans(hc_average_k3$cluster, data, "Hierarchical Average Linkage 3 Cluster Solution Developed PCA2 Data: Raw Score Means by Cluster")
```
...
```{r save objects}
save(data, km_k2_res, km_k3_res, pca_summary, pca_scaled, data_pca2components, variable_means_km2, variable_means_km3, file = "clustering_objects.Rda")
